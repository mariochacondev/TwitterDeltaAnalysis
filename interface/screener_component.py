...
class Screener(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self._headers = ["username", "tweeted", "tweet", "sentiment", "objectivity"]
        ...
        style.configure("Treeview", background=BG_COLOR, fieldbackground=BG_COLOR, foreground=FG_COLOR,  rowheight=30)
        ...
        self.tree = SortableTreeview(self, height=90)
        self.tree["columns"] = self._headers[1:]

        for col in self._headers[1:]:
            self.tree.column(col, anchor='w', width=85)
            self.tree.heading(col, text=col.capitalize(), anchor='w', sort_by="name")

        self.tree.column('#0', width=120)
        self.tree.heading('#0', text="Username", anchor='w', sort_by="name")
        self.tree.column('#1', width=135)
        self.tree.heading('#1', text="Created At", anchor='w', sort_by="name")
        self.tree.column('#2', width=1500)
        self.tree.heading('#2', text="Tweets", anchor='w', sort_by="name")



        # self.tree.bind('<Button-1>', self.handle_click)

        self.tree.pack(side=tk.TOP, pady=10)
